-- Create a file format for json folder
...
CREATE OR REPLACE stage MANAGE_DB.external_stages.json_folder	
	URL = "s3://bucketforsnowflakejd/json/" -- We paste the S3 URI from the bucket we need
	STORAGE_INTEGRATION = s3_int -- We paste the STORAGE INTEGRATION NAME for the bucket we created 
	FILE_FORMAT = MANAGE_DB.file_formats.json_fileformat; -- We paste the file format name that we created prepiously    

// Taming the JSON file

// First query from S3 Bucket   

SELECT * FROM @MANAGE_DB.external_stages.json_folder limit 10;
...
$1:summary,
$1:unixReviewTime
FROM @MANAGE_DB.external_stages.json_folder limit 10;
